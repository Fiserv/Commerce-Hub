...
The [status](#payment-url-status) of the Payment URL will be received in component `paymentUrlDetail` of the response.
...
Delete Payment URL request can be initiated by sending the request to the appropriate endpoint by providing valid `paymentUrlId` with no minimum field requirement.
