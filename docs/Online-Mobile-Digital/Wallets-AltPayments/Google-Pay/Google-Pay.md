...
## Accepting Google Pay Transactions
...
- [Google Play Services](https://developers.google.com/pay/api/android/guides/setup) version 18.0.0
...
The following parameters need to be defined in the SDK:
...
#### Fiserv Object Parameters

Parameters must be updated in the following files:

##### Constants.java

Update the Constants.java file with the Merchant ID and Gateway Tokenization parameters.

  - The Merchant ID will be shared by the Integration team and
  - The Gateway Tokenization parameter defaults to 'fiserv'.
...
<!-- theme: info -->
> To test a transaction the credit card information used in the app must be attached to an active account. The standard test cards will not be validated by Google and will fail in processing. The integration team will provide Google Pay specific test cards.
