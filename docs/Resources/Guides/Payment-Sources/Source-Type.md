...
|[Payment3DS](?path=docs/Online-Mobile-Digital/3D-Secure/Merchant-Managed-3DS.md) | **3-D Secure** authentication provides an additional security layer for online card transactions. If the merchant account is enabled for 3-D Secure, all charge transactions initiated by their API will by default go through the 3-D Secure process. Used to submit merchant managed 3-D Secure transaction to our application. |
