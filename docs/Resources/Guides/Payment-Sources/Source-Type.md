...
| ----- | ----- |
| PaymentCard | **Payment Cards** are issued by financial institutions and banks to the customers. Customers use the card to pay online or in-person. *PaymentCard* is used to submit a [payment card](?path=docs/Resources/Guides/Payment-Sources/Payment-Card.md), gift card, or [encrypted manual entry](?path=docs/In-Person/Encrypted-Payments/Manual.md) transaction to our application. |
