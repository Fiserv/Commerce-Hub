...
> :memo: **Note:** The same information (Activity and Declined) can be generated by the Reports --> Authorizations --> Generic Analysis menu item, however, the Commerce Hub transacting MIDs must be added to the 'Site Heirarchy' Filter or all merchant transactions will be included.
