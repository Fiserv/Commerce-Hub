...
Commerce Hub's Dynamic Currency Conversion *(DCC)* allows a merchant to obtain the issuing bank's rate by submitting the `bin` information in the request.
...
The example below contains the minimum [parameters](#parameters) for a successful DCC rate request using the cardholder's BIN. The full request schemas are available in our [API Explorer](../api/?type=post&path=/payments-vas/v1/accounts/currencies/dcc).
...
<!--
type: tab
titles: source, sourceCurrency, merchantDetails
-->
...
|---------|----------|----------------|---------|
| `sourceType` | *string* | 256 | Payment [source type](?path=docs/Resources/Guides/Payment-Sources/Source-Type.md) for a DCC request |
| `card` | *object* | N/A | Card specific data object |
| `bin` | *string* | 9 | Bank Identification Number (BIN), the initial set of four to nine numbers of the Primary Account Number (PAN) |
