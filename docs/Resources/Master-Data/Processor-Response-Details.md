...
| `hostResponseCode` | *string* |   | Endpoint or issuer host response code. See the specific processor's specification documentation for more details |
| `hostResponseMessage` | *string* |   | Endpoint or issuer host response message. See the specific processor's specification documentation for more details |
...
| `purchaseOrderRequiredIndicator` | *string* | 15 | It identifies if the `merchantOrderId` *(purchase order (PO) number)* is *REQUIRED* or *NOT_REQUIRED* |
| `taxExemptIndicator` | *string* | 32 | Identifies if the customer is tax *EXEMPT* or *NOT_EXEMPT* |
| `purchaseAprType` | *string* | 32 | Identifies if the annual percentage rate charged on the [private label](?path=docs/Resources/Guides/Payment-Sources/Private-Label.md) account is *FIXED* or *VARIABLE* |
| `arqcResponseCode` | *string* | 32 | An ARQC *(Authorization Request Cryptogram)* generated by the EMV card upon request from the device to obtain authorization for payment. ***Valid Values:** VALIDATION_FAILED, VALIDATION_UNAVAILABLE, UNAVAILABLE, VALIDATION_BYPASSED, or VALIDATION_PASSED* |
| `settlementIndicator` | *string* | N/A |  Returned for some [Fleet transactions](?path=docs/Resources/Guides/Payment-Sources/Fleet/Fleet.md) to indicate the transaction is eligible for *COST_PLUS*, *DIRECT_BILL*, or *FUNDED* settlement |
| `additionalInfo` | *array* | N/A | Contains custom name-value pairs based on merchant requirements, industry or vertical. |
| `bankAssociationDetails` | *object* | N/A | [Bank association](#bank-association-details) response details |
| `responseIndicators` | *object* | N/A | Debit [response indicators](#response-indicators) details |
...
| `postalCodeMatch` | *string* | 1 |Contains the normalized gateway response of [postal code](?path=docs/Resources/Guides/Fraud/Address-Verification.md#response-values) match |
| `securityCodeMatch` | *string* | 8 |  Contains the normalized gateway response of [security code](?path=docs/Resources/Guides/Fraud/Security-Code.md#response-values) match |
