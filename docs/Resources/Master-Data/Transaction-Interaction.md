...
| `mobileInteraction` | *string* | 12 | Identifies if the mobile interaction was *PHONE* or *QR_CODE*|
...
| `motoType` | *string* | N/A | Defines if the MOTO `origin` is *MAIL* or *PHONE* |
...
POS entry mode value identifies how the account number was entered on the transaction. The below table identifies the valid values of `posEntryMode`.
